...
def update_dynamo_db(zip, item_data):
    ...
    table.put_item(
        Item= {
            'zip': zip,
            'City': item_data['name'],
            'Weather': item_data["weather"][0]["description"],
            'Temperature': str(item_data["main"]["temp"]),
            'Pressure': str(item_data["main"]["pressure"]),
            'Humidity': str(item_data["main"]["humidity"]),
            'Wind_Speed': str(item_data["wind"]["speed"]),
        })
...
def lambda_handler(event, context):
    ...
    try:
        ...
        update_dynamo_db(zip, item_data)
